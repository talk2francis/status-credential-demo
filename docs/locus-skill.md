...
> **You are on:** `https://paywithlocus.com/` · `https://api.paywithlocus.com/api` | **Site map:** [llms.txt](https://paywithlocus.com/llms.txt)
>
...
| **Wrapped API index** | `https://paywithlocus.com/wapi/index.md` — see [wrapped APIs section](#wrapped-apis-pay-per-use-proxy) |
| **Wrapped API detail** | `https://paywithlocus.com/wapi/<provider>.md` — per-provider detail |
| **MPP service index** | `https://paywithlocus.com/discover/index.md` — MPP services (no account needed) |
| **MPP service detail** | `https://paywithlocus.com/discover/<provider>.md` — per-provider MPP detail |
| **llms.txt** | `https://paywithlocus.com/llms.txt` — complete site map for agents |
| **REQUEST_CREDITS.md** | `https://paywithlocus.com/request_credits.md` — request promotional USDC credits |
...
```bash
curl -s https://api.paywithlocus.com/api/x402/endpoints/md \
  -H "Authorization: Bearer YOUR_LOCUS_API_KEY" \
  -o X402ENDPOINTS.md
```
...
```bash
curl -X POST https://api.paywithlocus.com/api/x402/<slug> \
  -H "Authorization: Bearer YOUR_LOCUS_API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST https://api.paywithlocus.com/api/x402/call \
  -H "Authorization: Bearer YOUR_LOCUS_API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl "https://api.paywithlocus.com/api/x402/transactions?limit=50" \
  -H "Authorization: Bearer YOUR_LOCUS_API_KEY"
```
...
- **Index:** `https://paywithlocus.com/wapi/index.md`
- **Per-provider detail:** `https://paywithlocus.com/wapi/<provider>.md`

For example:
- Firecrawl (web scraping): `https://paywithlocus.com/wapi/firecrawl.md`
- Gemini (AI chat, vision, PDFs): `https://paywithlocus.com/wapi/gemini.md`
- OpenAI (GPT, images, audio, embeddings): `https://paywithlocus.com/wapi/openai.md`

**Only fetch the providers you actually need** to keep your context lean.

> **Also available via MPP (no account needed):** Providers with MPP support have discover pages at `https://paywithlocus.com/discover/<provider>.md` — see the [MPP section](#mpp-services) or `https://paywithlocus.com/llms.txt` for the full map.
...
| **Browse wrapped API providers** | `https://paywithlocus.com/wapi/index.md` | Index of all providers and endpoints |
| **Get provider detail** | `https://paywithlocus.com/wapi/<provider>.md` | Full detail for one provider (curl examples, params, costs) |
